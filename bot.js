...
const WORK_TYPE = config.WORK_TYPE === 'public';
...
const BOT_NAME_FANCY = 'DARK DEV MINI';
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      const pushname = msg.pushName || 'Shinigami User';
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2 restart ${process.env.PM2_NAME || 'SHINIGAMI-MINI'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
